...
class Dangnhap extends CI_Controller {
    ...
    public function dangky(){
        ...
        $todaylimit = strtotime("%Y-%m-%d", $todaylimit);
        ...
        $this->form_validation->set_rules('address', 'Địa chỉ chi tiết', 'required');
        $this->form_validation->set_rules('password', 'Mật khẩu', 'required|min_length[6]|max_length[32]');
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng 9 Orange,
             Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' ,
              Mã này có giá trị trong vòng 1 tháng kể từ ngày đăng ký,
    ...
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_user']    = 'hoangminhsang08121999@gmail.com';
            $config['smtp_pass']    = 'Hoangsang0812';
            $config['charset']    = 'utf-8';
            $config['newline']    = "\r\n";
            $config['wordwrap'] = TRUE;
            $config['mailtype'] = 'html';
            $config['validation'] = TRUE;   
            $this->email->initialize($config);
            $this->email->from('hoangminhsang08121999@gmail.com', '9 Orange');
